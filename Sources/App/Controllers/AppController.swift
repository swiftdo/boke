...
extension AppController {
    func getLearnUI(_ req: Request) throws -> EventLoopFuture<OutputJson<OutputAppLearn>> {
        return Booklet.query(on: req.db)
            .sort(\.$createdAt, .descending)
            .with(\.$author)
            .limit(5)
            .all()
            .map { booklets in
                return booklets.map{ booklet in OutputBooklet(booklet: booklet) }
        }.map{ booklets in
            return OutputJson(success: OutputAppLearn(booklets: booklets))
        }
    }

    func getQiNiuToken(_ req: Request) throws -> EventLoopFuture<OutputJson<String>> {
        guard let _ = req.auth.get(User.self) else {
            throw ApiError(code: OutputStatus.userNotExist)
        }
        return req.eventLoop.makeSucceededFuture(OutputJson(success: try QiniuService.token()))
    }
}
...
final class QiniuService {
    ...
    private func genery() throws -> String {
        ...
        let encodeString = Base64FS.encodeString(str: String(data: jsonData, encoding: .utf8) ?? "")

        let encodedSignedString = HMACSHA1(secretKey, text: encodeString)
        let token = accessKey + ":" + encodedSignedString + ":" + encodeString
        return token
    }

    private func HMACSHA1(_ key: String, text: String) -> String {
        let sign = HMAC<Insecure.SHA1>.authenticationCode(for:[UInt8](text.utf8), using: SymmetricKey(data: [UInt8](key.utf8)))
        return String(bytes: Base64FS.encode(data: [UInt8](sign)), encoding: .utf8) ?? ""
    }
}
